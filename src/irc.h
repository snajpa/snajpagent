...
enum snag_irc_event_kind {
    SNAG_IRC_CONNECTED, SNAG_IRC_DISCONNECTED, SNAG_IRC_JOIN, SNAG_IRC_PART,
...
int snag_irc_remove(struct snag_irc *irc, bool hosting, const char *endpoint, char *error, size_t error_size);
...
/* Monotonic count of destination-set mutations; advances when an owner view, room, target
 * or endpoint changes, and when owners are added or removed. */
