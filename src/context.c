...
append_goal_controller(struct context_builder *builder)
{
    ...
    if (!snag_goal_unfinished(builder->session->goal_status)) {
        return append_message(builder, "system",
...
append_rollout_log_location(struct context_builder *builder)
{
    ...
    if (quoted_path)
        rc = append_systemf(builder, quoted_path_max + 256u,
...
tool_schema(const char *name, const char *required_keys, const char *description, json_t *properties)
{
    ...
    for (void *iter = json_object_iter(properties); iter;
        ...
        if (snag_string_in(json_object_iter_key(iter), required_keys) &&
            json_array_append_new(required, json_string(json_object_iter_key(iter))) < 0)
            goto out;
             ...
             bool read_only)
{
    ...
    if (json_array_append_new(tools,
        ...
        goto fail;
    if (networked &&
        (json_array_append_new(tools, tool_schema("irc_send", "text",
                  ...
                  char *error, size_t error_size)
{
    ...
    if (!session || !model || !effort || !steering ||
        ...
        append_message(&builder, "system", harness) < 0 ||
        (builder.networked &&
