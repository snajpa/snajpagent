...
{
    if (!builder->input_timed && !first) return append_message(builder, "user", text);
...
compact_complete_boundary(struct context_builder *builder, uint64_t seq, char *error, size_t error_size)
{
    ...
    if (source_bytes <= builder->compact_budget || (!builder->compact_best_known &&
...
static int
steering_matches_snapshot(struct context_builder *builder, const char *id, const char *text)
{
    json_t *item;
    const char *snap_id;
    ...
    if (!snap_id || !snap_text || strcmp(snap_id, id) != 0 || strcmp(snap_text, text) != 0) return 0;
