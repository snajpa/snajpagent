...
static json_t *
excerpt_json(struct snag_buf *text, const char *label, const struct output_excerpt *stream)
{
...
remove_env_entry(const char *entry, const struct snag_config *config)
{
    ...
    if (snag_environment_prefix(entry, "SNAJPAGENT_")) return true;
    return false;
}
...
{
    if (proc->closing || process_ready(proc)) return;
    ...
    proc->deadline_ms = saturating_deadline(snag_monotonic_ms(), SNAG_TOOL_CLOSE_GRACE_MS);
}
    ...
    if (proc->input_written == proc->input.len) {
        ...
        if (proc->input_eof && proc->stdin_open) {
            if (proc->child.pty && !proc->pty_eof_sent) {
                size_t written = 0u;
                write_stdin_chunk(proc, "\004", 1u, &written, &proc->stdin_open);
                proc->pty_eof_sent = written == 1u;
                if (!proc->pty_eof_sent) return;
            }
            managed_close_input(proc);
        }
    }
...
snag_tools_service(int timeout_ms, snag_wake_fd wake_fd, char *error, size_t error_size)
{
    ...
    for (size_t j = 0u; j < streams && serviced < 16u; ++j) {
        ...
        if (fds[i].revents & SNAG_CHILD_ERROR) goto fail;
...
snag_tools_collect(const char *handle, const char *reason, json_t **result, char *error, size_t error_size)
{
    ...
    if (process_ready(proc)) {
        ...
        if (!proc->child.reaped) {
            snag_child_signal(&proc->child, SNAG_CHILD_KILL);
            if (snag_child_reap(&proc->child) < 0) goto out;
        }
    ...
    *result = result_json(status, reason, exit_code, signal_number,
    ...
    rc = 0;
out:
    for (unsigned int s = 0u; s < 2u; ++s) snag_buf_free(&streams[s].data);
    if (rc < 0 && *result) {
        json_decref(*result);
        *result = NULL;
    }
                         ...
                         const struct snag_credential *credential, char *error, size_t error_size)
{
    ...
    if (snag_secret_set_build(&proc->secrets, config, credential, error, error_size) < 0) goto out;
    for (size_t i = 0u; i < proc->secrets.wire.count; ++i) {
        size_t len = strlen(proc->secrets.values[i]);
        if (len > proc->max_secret) proc->max_secret = len;
    }
    ...
    if (stdin_len && snag_buf_append(&proc->input, stdin_text, stdin_len) < 0) goto out;
    ...
    *result = NULL;
    ...
    if (args.exec) {
        ...
        if (args.terminate) {
            ...
            if (len && snag_buf_append(&proc->input, args.input, len) < 0) return -1;
                        ...
                        json_t **result, char *error, size_t error_size)
{
    ...
    return wait_process(handle, 0u, pump, pump_opaque, wake_fd, result, error, error_size);
}
