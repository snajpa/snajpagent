...
accounting_valid(const json_t *model)
{
    ...
    return state && (snag_string_in(state, "unknown supported unsupported")) &&
           snag_json_integer_u64(model, "observed_hard_input_tokens", &hard) == 0 &&
           snag_json_integer_u64(model, "observed_input_tokens", &tokens) == 0 &&
           snag_json_integer_u64(model, "observed_input_bytes", &bytes) == 0 &&
           hard <= SNAG_CONFIG_TOKEN_LIMIT_MAX && tokens <= SNAG_CONFIG_TOKEN_LIMIT_MAX &&
...
provider_entry(const json_t *providers, const char *name)
{
    for (size_t i = 0; providers && i < json_array_size(providers); ++i) {
        ...
        if (strcmp(snag_json_string(entry, "name"), name) == 0) return entry;
    }
    return NULL;
}
                     ...
                     char *error, size_t error_size)
{
    ...
    if (!store || store->root_fd < 0 || !cache) return snag_errno(EINVAL);
                        ...
                        char *error, size_t error_size)
{
    ...
    if (snag_model_cache_load(store, &previous, error, error_size) < 0 && errno != EINVAL) goto out;
...
const json_t *
snag_model_cache_find(const struct snag_model_cache *cache, const char *provider, const char *model)
{
    const json_t *entry;
    json_t *models;

    if (!cache || !cache->providers || !provider || !model) return NULL;
    ...
    for (size_t i = 0; i < json_array_size(models); ++i)
        if (strcmp(snag_json_string(json_array_get(models, i), "id"), model) == 0)
              ...
              const char *effort, const json_t *metadata)
{
    ...
    (void)metadata;
    if (index != selection->index) return 0;
    *selection->provider = provider;
                           ...
                           char *error, size_t error_size)
{
    ...
    /* The provider's normal working window is the default budget. An advertised
     * maximum is permission to select more, not a default choice: relocating a
     * request above the normal window can move it into a provider's higher
