        ...
        size_t cells, bool single_line, bool *truncated)
{
    ...
    for (size_t i = 0u; i < len;) {
        ...
        if (c < 0x20u || c == 0x7fu) {
            ...
            snag_buf_reset(&safe);
            if (snag_term_append_safe(&safe, piece, n) < 0) goto out;
        ...
        if (count > characters - used || w > cells - width || bytes > out->max - out->len) {
            *truncated = true;
            break;
        }
        if (snag_buf_append(out, piece, bytes) < 0) goto out;
        used += count;
        width += w;
        i += n;
    }
                               ...
                               unsigned int level, unsigned int columns)
{
    ...
    if (ref) handle = snag_json_string(ref, "handle");
