...
static int
ensure_directory(const char *path, bool require_private, char *error, size_t error_size)
{
    snag_file_info st;
    ...
    if (require_private) {
        ...
        errno = saved;
        if (rc < 0) return snag_errorf(error, error_size, "%s must be private (mode 0700)", path);
    }
    return 0;
}
static int
...
{
    if (store->trash_fd >= 0) (void)close(store->trash_fd);
    if (store->sessions_fd >= 0) (void)close(store->sessions_fd);
    if (store->root_fd >= 0) (void)close(store->root_fd);
    free(store->root_path);
    snag_store_init(store);
}
int
snag_store_open(struct snag_store *store, const char *dotdir, char *error, size_t error_size)
{
    char *sessions = NULL;
    char *trash = NULL;
    int rc = -1;
                   ...
                   uint64_t *written_seq, char *error, size_t error_size)
{
    ...
    if (session->pending_log) {
        if (snag_buf_append(session->pending_log, line.data, line.len) < 0) goto memory_error;
    } else {
        ...
        if (snag_write_full(session->log_fd, line.data, line.len) < 0 ||
            ...
            if (session->append_rollback_pending && session->append_rollback_end >= session->log_end &&
                ...
                session->append_rollback_end = snag_seek(session->log_fd, 0, SEEK_END);
            snag_errorf(error, error_size, "cannot durably append %s: %s", type, strerror(saved));
            errno = saved;
            goto out;
        }
    }
...
replace_text(struct snag_session *session, const char **slot, const char *key, const char *text, size_t max)
{
    ...
    if (snag_json_set_new(session->strings, key, json_string(text)) < 0) return -1;
...
clear_response_state(struct snag_session *session)
{
    ...
    session->response_handoff = false;
...
all_pending_finished(const struct snag_session *session)
{
    for (size_t i = 0; i < session->pending_call_count; ++i)
        if (!session->pending_calls[i].finished) return false;
    return true;
}
...
snag_session_process(struct snag_session *session, const char *handle)
{
    for (size_t i = 0u; i < session->process_count; ++i)
        if (handle && !strcmp(session->processes[i].handle, handle)) return &session->processes[i];
    return NULL;
}
...
compact_output_digest(const json_t *output, char out[SNAG_SHA256_HEX_LEN + 1u], size_t *bytes)
{
    ...
    return snag_json_digest_bounded(output, 12u * 1024u * 1024u, out, bytes);
}
static bool
...
{
    for (size_t i = 0; i < session->pending_steering_count; ++i)
        if (strcmp(session->pending_steering[i].steering_id, id) == 0) return true;
    for (size_t i = 0; i < session->pending_queue_count; ++i)
        if (strcmp(session->pending_queue[i].queue_id, id) == 0) return true;
    return false;
}
static int
...
consume_oldest_queue(struct snag_session *session)
{
    ...
    if (session->pending_queue_count > 1u) memmove(&session->pending_queue[0], &session->pending_queue[1],
            ...
            uint64_t seq, char *error, size_t error_size)
{
    ...
    if (strcmp(type, "session_created") == 0) {
        ...
        static const char reasons[] = "manual proactive hard_budget provider_rejection";
        ...
        if (!snag_json_exact_keys(data, "compact_id reason") ||
            ...
            strcmp(compact_id, session->active_compact_id) != 0 || !snag_string_in(reason, reasons))
            goto invalid;
        clear_compaction_state(session);
    } else if (strcmp(type, "compaction_completed") == 0) {
        static const char methods[] = "exact unknown statistical_upper_estimate qualified_upper_bound";
        const char *compact_id = snag_json_string(data, "compact_id");
        const char *method = snag_json_string(data, "count_method");
        const char *output_method = snag_json_string(data, "output_count_method");
        const char *output_count_hash = snag_json_string(data, "output_count_request_sha256");
        const char *source_hash = snag_json_string(data, "source_sha256");
        ...
        if (json_object_get(data, "received_at_ms") && snag_json_integer_u64(data, "received_at_ms",
        ...
        for (size_t i = 0u; i < session->pending_queue_count; ++i) {
            ...
            if (id && !strcmp(queued->queue_id, id)) {
                ...
                if (out != i) session->pending_queue[out] = *queued;
                ++out;
            }
        }
        ...
        if (session->active_turn || session->process_count != 0u || session->pending_steering_count != 0u ||
        ...
        if (goal) {
            ...
            if (session->pending_queue_count == 0u || !session->pending_queue[0].text || !queue_id ||
                ...
                strcmp(text, session->pending_queue[0].text) != 0 || session->pending_queue[0].read_only !=
                    json_is_true(json_object_get(data, "read_only")) || strlen(text) > SNAG_MAX_QUEUED_TEXT)
                goto invalid;
        }
        ...
        const char *message = snag_json_string(data, "message");
        const char *provider_source_sha256 = snag_json_string(data, "provider_source_sha256");
        const char *request_hash = snag_json_string(data, "request_sha256");
        json_t *observed_ceiling = json_object_get(data, "observed_hard_input_tokens");
        ...
        if (decision.outcome == SNAG_GRAPH_FINAL || decision.outcome == SNAG_GRAPH_REFUSAL) {
            ...
            const struct snag_response_item *item = &view;
            memcpy(session->final_item_id, item->local_item_id, sizeof(session->final_item_id));
            memcpy(session->final_response_id, response_id, sizeof(session->final_response_id));
        }
               ...
               char *error, size_t error_size)
{
    ...
    for (;;) {
        ...
        if (got == 0) {
            if (boundary >= 0 && read_off != boundary) goto boundary_error;
            break;
        }
        read_off += got;
    ...
    if (complete_end_out) *complete_end_out = complete_end;
    if (next_seq_out) *next_seq_out = seq;
    rc = 0;
    goto out;
                       ...
                       void *opaque, char *error, size_t error_size)
{
    ...
    if (!session || !fn || (!session->pending_log && session->log_fd < 0) || session->log_end < 0 ||
