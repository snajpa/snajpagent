...
{
    (void)id;
    if(state==RTC_GATHERING_COMPLETE)atomic_store(&((struct snag_voice_rtc *)opaque)->gathered,true);
}
static void connection(int id,rtcState state,void *opaque)
{
    (void)id;
    if(state==RTC_FAILED || state==RTC_CLOSED)atomic_store(&((struct snag_voice_rtc *)opaque)->failed,true);
}
...
int snag_voice_rtc_open(struct snag_voice_rtc **out,char *error,size_t size)
{
    ...
    *out=r;return 0;
fail:snag_voice_rtc_close(r);return snag_fail(error,size,EIO,"Cannot initialize native voice media transport");
}
int snag_voice_rtc_offer(struct snag_voice_rtc *r,struct snag_buf *out)
{
    ...
    if(n<=0 || (size_t)n>sizeof(sdp) || snag_buf_append(out,sdp,strlen(sdp))<0 || snag_buf_terminate(out)<0)return -1;
    return 1;
}
int snag_voice_rtc_answer(struct snag_voice_rtc *r,const char *sdp)
{ return rtcSetRemoteDescription(r->pc,sdp,"answer"); }
bool snag_voice_rtc_ready(struct snag_voice_rtc *r) { return !atomic_load(&r->failed) && rtcIsOpen(r->track); }
...
{*out=NULL;return snag_fail(e,n,ENOTSUP,"Native voice is unavailable in this audio-disabled build");}
...
int snag_voice_rtc_input(struct snag_voice_rtc *r,const int16_t *p,uint32_t n) {(void)r;(void)p;(void)n;return -1;}
int snag_voice_rtc_output(struct snag_voice_rtc *r,int16_t *p,uint32_t n) {(void)r;(void)p;(void)n;return -1;}
