...
enum snag_presentation {
    ...
    SNAG_PRESENT_OUTPUT, SNAG_PRESENT_CONTEXT, SNAG_PRESENT_DEBUG, SNAG_PRESENT_PROTOCOL, SNAG_PRESENT_WIRE,
    SNAG_PRESENT_CHAT, SNAG_PRESENT_FEEDBACK };

bool snag_presentation_enabled(enum snag_presentation kind, unsigned int level, enum snag_render_view view);
...
int snag_render_public(struct snag_render *render, const char *text, size_t len, struct snag_buf *delivered);
...
int snag_render_rollout(struct snag_render *render, const char *text, size_t len, struct snag_buf *delivered);
