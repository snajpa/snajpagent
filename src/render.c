...
bool
snag_presentation_enabled(enum snag_presentation kind, unsigned int level, enum snag_render_view view)
{
    static const unsigned char minimum[] = {0, 1, 2, 2, 3, 4, 5, 6, 0, 0};
    if ((unsigned int)kind >= sizeof(minimum) || level < minimum[kind]) return false;
    if (kind == SNAG_PRESENT_FEEDBACK) return true;
...
{
    if (!snag_presentation_enabled(kind, level, SNAG_RENDER_ROLLOUT)) return 0u;
    if (level == 2u && kind == SNAG_PRESENT_ARGUMENTS) return 1024u;
    if (level == 2u && kind == SNAG_PRESENT_OUTPUT) return 512u;
    return SIZE_MAX;
}
...
{
    if (render->suppress_optional && kind >= SNAG_PRESENT_TOOL && kind <= SNAG_PRESENT_WIRE) return false;
...
{
    if (!render->checkpoint) return 0;
    if (!render->markdown_measuring && close_public_output(render) < 0) return -1;
...
text_slice(const char *text, size_t len)
{
    ...
    while (amount < len && amount && ((unsigned char)text[amount] & 0xc0u) == 0x80u) --amount;
...
static int
boundary_before(struct snag_render *render, int fd, unsigned int kind, const char *text, size_t len)
{
    if (!(fd == STDOUT_FILENO ? render->stdout_terminal : render->stderr_terminal) ||
        render->markdown_measuring) return 0;
    size_t i = 0u;
    while (i < len && text[i] == '\n') ++i;
    if (i == len) return 0;
                 ...
                 const char *text, size_t len, size_t colored_len, bool terminal_safe, bool persistent)
{
    ...
    if (colored_len > len) return snag_errno(EINVAL);
    if (!len) return 0;
    ...
    if ((fd == STDOUT_FILENO && render->stdout_terminal) || (fd == STDERR_FILENO && render->stderr_terminal))
                 ...
                 const char *text, size_t len, size_t colored_len, bool terminal_safe, bool persistent)
{
    ...
    if (len && render->public_item_open && render->wrap_pending.len &&
        (flush_wrap_pending(render) < 0 || close_public_output(render) < 0)) return -1;
    if (prose) {
        if (markdown_gap(render) < 0 || close_public_output(render) < 0) return -1;
    }

    if (!len) return write_role_chunk(render, boundary, fd, color, text, len, colored_len,
                                 terminal_safe, persistent);
    while (len) {
        size_t amount = text_slice(text, len);
        size_t colored = colored_len < amount ? colored_len : amount;
        if (render->term) render->term->defer_redraw = deferred || len > amount;
    ...
    if (prose && rc == 0 && (markdown_gap(render) < 0 || close_public_output(render) < 0)) rc = -1;
    if (render->term) render->term->defer_redraw = deferred;
    return rc;
}
...
write_omitted(struct snag_render *render)
{
    ...
    return write_role_block(render, BOUNDARY_CONTENT, STDERR_FILENO, COLOR_META,
                            marker, sizeof(marker) - 1u, sizeof(marker) - 1u, render->stderr_terminal, true);
}

static int
write_optional_block(struct snag_render *render, enum snag_presentation kind,
                     const char *color, const char *text, size_t len, size_t colored_len)
{
    bool ended_lf = true;
    bool wrote = false;
    int rc = -1;
    ...
    while (len) {
        ...
        if (write_role_chunk(render, BOUNDARY_CONTENT, STDERR_FILENO, color, text, amount,
                              colored, render->stderr_terminal, true) < 0) goto out;
        wrote = true;
...
static void
queue_record(struct snag_render *render, enum snag_render_view view, struct snag_render_record *record)
{
    if (render->view_tail[view]) render->view_tail[view]->next = record;
    else render->view_head[view] = record;
...
pop_record(struct snag_render *render, enum snag_render_view view)
{
    ...
    if (!render->view_head[view]) render->view_tail[view] = NULL;
    if (render->rollout_open == record) render->rollout_open = NULL;
    free_record(record);
}
...
snag_render_set_color(struct snag_render *render, enum snag_color_mode mode)
{
    ...
    bool disabled = mode == SNAG_COLOR_NEVER || (mode == SNAG_COLOR_AUTO &&
                     (getenv("NO_COLOR") != NULL || !term || strcmp(term, "dumb") == 0));

    render->color_stdout = !disabled && (mode == SNAG_COLOR_ALWAYS || render->stdout_terminal);
    render->color_stderr = !disabled && (mode == SNAG_COLOR_ALWAYS || render->stderr_terminal);
    if (render->term) snag_term_set_color(render->term, render->color_stderr);
}
             ...
             const char *text, size_t len, size_t colored_len)
{
    ...
    if (rc == 0) rc = write_role_block(render, boundary, STDERR_FILENO, color,
...
{
    return write_banner(render, BOUNDARY_CONTENT, COLOR_HOST, text, strlen(text), strlen(text));
}
...
render_history_turn(struct snag_render *render, const struct snag_history_turn *turn)
{
    ...
    if (rc == 0 && assistant) {
        if (snag_render_public_begin(render, STDERR_FILENO, "assistant: ") < 0) {
            rc = -1;
        } else if (snag_render_public(render, assistant, strlen(assistant), NULL) < 0) {
...
snag_render_submitted(struct snag_render *render, const char *label, const char *text)
{
    ...
    if (render->term && snag_term_consume_echoed_submission(render->term, label)) {
        ...
        if (rc == 0) rc = snag_buf_append(&line, label, strlen(label));
        if (rc == 0) rc = snag_buf_append(&line, text, len);
    ...
    if (rc == 0 && terminal) {
        ...
        if (render->term) render->term->output_gap = 1u;
...
int
snag_render_input_submitted(struct snag_render *render, const char *label, const char *text)
{
...
snag_render_before_prompt(struct snag_render *render)
{
    ...
    if (!render->previous_public_item) return 0;
    if (!render->stderr_terminal) {
        render->previous_public_item = false;
        return 0;
    }
    count = render->trailing_newlines < 2u ? 2u - render->trailing_newlines : 0u;
    if (count && write_block(render, STDERR_FILENO, newlines, count, false, true) < 0) return -1;
    render->previous_public_item = false;
    return 0;
}

static int public_write(struct snag_render *render, const char *text, size_t len, unsigned char style);
static int markdown_finish(struct snag_render *render);
static int markdown_abort(struct snag_render *render);
static int markdown_write(struct snag_render *render, const unsigned char *text, size_t len);
static int markdown_table_finish(struct snag_render *render);
static bool public_terminal(const struct snag_render *render);

enum markdown_style {
    MD_HEADING = 1u, MD_QUOTE = 2u, MD_STRONG = 4u, MD_EMPHASIS = 8u,
    MD_STRIKE = 16u, MD_CODE = 32u, MD_LINK = 64u };

static unsigned char
markdown_style(const struct snag_render *render)
{
    const struct snag_markdown_state *md = &render->markdown_state;
    if (!render->markdown_rendering || !color_enabled(render, render->public_fd)) return 0u;
    return (md->heading ? MD_HEADING : 0u) | (md->quote ? MD_QUOTE : 0u) |
           ((md->strong || md->table_header) ? MD_STRONG : 0u) | (md->emphasis ? MD_EMPHASIS : 0u) |
           (md->strike ? MD_STRIKE : 0u) | ((md->inline_code || md->fence) ? MD_CODE : 0u) |
           (md->link_url ? MD_LINK : 0u);
}
...
static int
markdown_paint_style(struct snag_render *render, unsigned char style)
{
    char sequence[64];
    size_t len = 3u;

    if (!style) return markdown_clear_style(render);
...
#define ADD_STYLE(value) do { \
    ...
    if (style & MD_HEADING) ADD_STYLE(";1;36");
    if (style & MD_QUOTE) ADD_STYLE(";34");
    if ((style & MD_STRONG) && !(style & MD_HEADING)) ADD_STYLE(";1");
    if (style & MD_EMPHASIS) ADD_STYLE(";3");
    if (style & MD_STRIKE) ADD_STYLE(";2");
    if (style & MD_CODE) ADD_STYLE(";33");
    if (style & MD_LINK) ADD_STYLE(";4;34");
#undef ADD_STYLE
    sequence[len++] = 'm';
    sequence[len] = '\0';
...
snag_render_public_begin(struct snag_render *render, int fd, const char *label)
{
    ...
    if (render->public_item_open || render->utf8_pending_len || render->wrap_pending.data)
        return snag_errno(EBUSY);
    if (fd != STDOUT_FILENO && fd != STDERR_FILENO) return snag_errno(EINVAL);
    terminal = fd == STDOUT_FILENO ? render->stdout_terminal : render->stderr_terminal;
    if (render->markdown && terminal && render->previous_public_item &&
        ...
        if (write_block(render, fd, newlines, count, false, true) < 0) return -1;
    }
    render->previous_public_item = false;
    ...
    if (render->public_column == SIZE_MAX) goto fail;
    if (label_len) {
...
{
    return render->public_fd == STDOUT_FILENO ? render->stdout_terminal : render->stderr_terminal;
}

static int
public_write(struct snag_render *render, const char *text, size_t len, unsigned char style)
{
    bool terminal = public_terminal(render);

    if (!len) return 0;
    if (render->markdown_measuring) return 0;
    ...
    if (terminal) render->trailing_newlines = trailing_newlines(render->trailing_newlines, text, len);
    return 0;
}
...
close_public_output(struct snag_render *render)
{
    ...
    if (!render->public_output_open) return 0;
...
static int
write_styled_span(struct snag_render *render, const char *text, const unsigned char *styles, size_t len)
{
    while (len) {
...
flush_wrap_pending(struct snag_render *render)
{
    ...
    if (render->wrap_has_word && !render->wrap_continuation &&
        columns >= 20u && render->public_column > indent &&
        (width >= columns || render->public_column > columns - width)) {
        if (public_write(render, "\n", 1u, 0u) < 0) return -1;
        render->public_column = 0u;
        text += leading;
        len -= leading;
        if (render->markdown_rendering && render->markdown_state.prose && render->markdown_prose_bullets) {
            if (public_write(render, "  ", 2u, 0u) < 0) return -1;
            render->public_column = 2u;
        }
        width = snag_term_text_width(text, len);
        if (width == SIZE_MAX) return -1;
    }
    /* Overlong words stream in bounded pieces. Include combining marks with
     * their base and materialize margin wraps with the same prose indentation. */
    if (columns >= 20u && (render->public_column >= columns || width > columns - render->public_column)) {
        while (len) {
            ...
            while (used < len) {
                ...
                if (w > room - cells) break;
                used += n;
                cells += w;
            }
            if (used && write_wrap_span(render, text, used) < 0) return -1;
            render->public_column += cells;
            text += used;
            len -= used;
            if (len) {
                if (public_write(render, "\n  ", 1u + indent, 0u) < 0) return -1;
                render->public_column = indent;
            }
        }
        width = 0u;
    } else if (write_wrap_span(render, text, len) < 0) {
        return -1;
    }
    if (render->public_column > SIZE_MAX - width) return snag_errno(EOVERFLOW);
    if (columns >= 20u) {
        size_t total = render->public_column + width;

        render->public_column = total > columns ? total % columns : total;
        if (total > columns && render->public_column == 0u) render->public_column = columns;
    } else {
...
write_wrapped(struct snag_render *render, const unsigned char *text, size_t len)
{
    ...
    for (size_t i = 0u; i < len;) {
        ...
        if (text[i] == '\n') {
            ...
            render->wrap_styles.len += n;
            render->wrap_has_word |= !space;
            size_t width = snag_term_text_width((const char *)text + i, n);
            if (width == SIZE_MAX || (!space &&
                !snag_size_add(render->wrap_width, width, &render->wrap_width))) return -1;
            /* A whole fitting word waits for its boundary, even across SSE and
             * style changes. Bound long words/combining sequences independently
             * of provider chunk size; never retain a paragraph. */
            if (render->wrap_width > limit || render->wrap_pending.len >= 4096u) {
                if (flush_wrap_pending(render) < 0) return -1;
                render->wrap_continuation = !space;
            }
        }
        i += n;
    }
...
{
    return len != 1u || (text[0] >= '0' && text[0] <= '9') || (text[0] >= 'A' && text[0] <= 'Z') ||
...
markdown_text(struct snag_render *render, const void *text, size_t len)
{
    ...
    if (!len) return 0;
    if (write_wrapped(render, bytes, len) < 0) return -1;
    last = len - 1u;
    while (last && (bytes[last] & 0xc0u) == 0x80u) --last;
    render->markdown_state.previous_word = markdown_word(bytes + last, len - last);
    return 0;
}
...
markdown_paragraph(struct snag_render *render, bool prose)
{
    ...
    if (md->prose == prose) return 0;
    if (flush_wrap_pending(render) < 0) return -1;
    md->prose = prose;
    md->line_continuation = false;
    if (!render->markdown_prose_bullets || render->markdown_measuring) return 0;
...
markdown_repeat(struct snag_render *render, char value, size_t count)
{
    ...
    while (count) {
        ...
        if (markdown_text(render, text, amount) < 0) return -1;
        count -= amount;
    }
    return 0;
}
...
static int
markdown_flush_delimiter(struct snag_render *render, bool next_word, bool at_end)
{
    struct snag_markdown_state *md = &render->markdown_state;
    ...
    md->delimiter = '\0';
    md->delimiter_len = 0u;
    if (md->inline_code && delimiter != '`') return markdown_repeat(render, delimiter, count);
    if (delimiter == '`') {
        if (!md->inline_code) {
            if (at_end) return markdown_repeat(render, delimiter, count);
            md->inline_code = true;
    ...
    if (delimiter == '~') {
        if (at_end && (!md->strike || !((count / 2u) & 1u))) return markdown_repeat(render, delimiter, count);
        if (count & 1u && markdown_repeat(render, '~', 1u) < 0) return -1;
        if ((count / 2u) & 1u) md->strike = !md->strike;
        return 0;
    }
    if (delimiter == '_' && md->delimiter_previous_word && next_word)
        return markdown_repeat(render, delimiter, count);
    if (at_end && (((count / 2u) & 1u && !md->strong) || (count & 1u && !md->emphasis)))
        return markdown_repeat(render, delimiter, count);
    if ((count / 2u) & 1u) md->strong = !md->strong;
    if (count & 1u) md->emphasis = !md->emphasis;
    return 0;
}
...
markdown_inline(struct snag_render *render, const unsigned char *text, size_t len)
{
    ...
    while (i < len) {
        ...
        if (md->escape) {
            md->escape = false;
            if (!(n == 1u && markdown_escapable(text[i])) && markdown_text(render, "\\", 1u) < 0) return -1;
            if (markdown_text(render, text + i, n) < 0) return -1;
            i += n;
            continue;
        }
        if (md->link_after_label) {
            ...
            if (n == 1u && text[i] == '(') {
                if (markdown_text(render, " <", 2u) < 0) return -1;
                md->link_url = true;
                i += n;
                continue;
            }
        ...
        if (md->link_url) {
            if (n == 1u && text[i] == ')') {
                md->link_url = false;
                if (markdown_text(render, ">", 1u) < 0) return -1;
        ...
        if (md->delimiter_len && !(n == 1u && text[i] == (unsigned char)md->delimiter) &&
            markdown_flush_delimiter(render, word, false) < 0) return -1;
        if (n == 1u && text[i] == '\\' && !md->inline_code) {
            md->escape = true;
            i += n;
            continue;
        }
        if (n == 1u && (text[i] == '`' || (!md->inline_code && (text[i] == '*' || text[i] == '_' ||
                                  text[i] == '~')))) {
            if (!md->delimiter_len) {
        ...
        }
        {
            ...
            do {
                i += n;
                if (i >= len || i - start >= 1024u) break;
                n = snag_utf8_size(text[i]);
                if (!n || n > len - i) break;
            } while (!(n == 1u && (text[i] == '\\' || text[i] == '`' || text[i] == ']' ||
                        (!md->inline_code && (text[i] == '*' || text[i] == '_' || text[i] == '~')))));
            if (markdown_text(render, text + start, i - start) < 0) return -1;
        }
    }
    return 0;
}
...
markdown_table_trim(struct markdown_table_cell *cell)
{
    ...
    while (cell->len && (cell->text[cell->len - 1u] == ' ' || cell->text[cell->len - 1u] == '\t'))
        --cell->len;
}

static bool
markdown_table_cells(const unsigned char *text, size_t len,
                     struct markdown_table_cell cells[MARKDOWN_TABLE_COLUMNS], size_t *cell_count)
{
    size_t first = 0u;
    size_t start;
    size_t end = len;
    ...
    while (first < len && first < 3u && text[first] == ' ') ++first;
    if (first >= len || text[first] != '|') return false;
    while (end > first && (text[end - 1u] == ' ' || text[end - 1u] == '\t')) --end;
    start = first + 1u;
    for (size_t i = start; i < end;) {
        ...
        if (text[i] == '`') {
            ...
            while (i + ticks < len && text[i + ticks] == '`') ++ticks;
            if (!code_ticks) code_ticks = ticks > UINT_MAX ? UINT_MAX : (unsigned int)ticks;
            else if (ticks == code_ticks) code_ticks = 0u;
            i += ticks;
            continue;
        }
        if (!code_ticks && text[i] == '|') {
            if (count == MARKDOWN_TABLE_COLUMNS) return false;
...
static bool
markdown_table_delimiter(const struct markdown_table_cell *cells, size_t count,
                         enum markdown_table_alignment *alignment)
{
    for (size_t i = 0u; i < count; ++i) {
        ...
        if (right) --end;
        if (end - begin < 3u) return false;
        for (size_t j = begin; j < end; ++j)
            if (text[j] != '-') return false;
        alignment[i] = left && right ? TABLE_CENTER : right ? TABLE_RIGHT : TABLE_LEFT;
    }
    return true;
}
...
markdown_inline_tail(struct snag_render *render)
{
    ...
    if (markdown_flush_delimiter(render, false, true) < 0) return -1;
    if (md->escape && markdown_text(render, "\\", 1u) < 0) return -1;
    md->escape = false;
    md->link_after_label = false;
    if (md->link_url) {
        md->link_url = false;
...
static int
markdown_table_rule(struct snag_render *render, const char *left, const char *middle, const char *right,
                    const size_t *widths, size_t count, bool newline)
{
    if (markdown_text(render, left, strlen(left)) < 0) return -1;
    for (size_t i = 0u; i < count; ++i) {
        for (size_t j = 0u; j < widths[i] + 2u; ++j)
            if (markdown_text(render, "─", strlen("─")) < 0) return -1;
        if (i + 1u < count && markdown_text(render, middle, strlen(middle)) < 0) return -1;
    }
    if (markdown_text(render, right, strlen(right)) < 0 || (newline && markdown_text(render, "\n", 1u) < 0))
        return -1;
    return 0;
}

static int
markdown_table_cell(struct snag_render *render, const struct markdown_table_cell *cell, bool strong)
{
    render->markdown_state.previous_word = false;
    render->markdown_state.table_header = strong;
    if (markdown_inline(render, cell->text, cell->len) < 0) return -1;
...
static int
markdown_table_grid_row(struct snag_render *render, const struct markdown_table_cell *cells,
                        size_t cell_count, const size_t *widths,
                        const enum markdown_table_alignment *alignment, size_t columns, bool header)
{
...
static bool
markdown_table_next_line(const unsigned char *text, size_t len, size_t *offset, const unsigned char **line,
                         size_t *line_len)
{
    size_t start;
    size_t end;
...
markdown_table_render(struct snag_render *render)
{
    ...
    if (!markdown_table_next_line(text, md->table.len, &offset, &line, &line_len) ||
        !markdown_table_cells(line, line_len, header, &header_count) ||
        !markdown_table_next_line(text, md->table.len, &offset, &line, &line_len) ||
        !markdown_table_cells(line, line_len, delimiter, &delimiter_count) ||
        delimiter_count != header_count || !markdown_table_delimiter(delimiter, delimiter_count, alignment))
        return snag_errno(EINVAL);
    for (size_t i = 0u; i < header_count; ++i) {
        if (markdown_table_cell_width(render, &header[i], &widths[i], &minimum[i]) < 0) return -1;
        if (!widths[i]) widths[i] = 1u;
        if (!minimum[i]) minimum[i] = 1u;
    }
    body_offset = offset;
    while (markdown_table_next_line(text, md->table.len, &offset, &line, &line_len)) {
        if (render_checkpoint(render) < 0) return -1;
        struct markdown_table_cell cells[MARKDOWN_TABLE_COLUMNS];
        size_t count;

        if (!markdown_table_cells(line, line_len, cells, &count)) return snag_errno(EINVAL);
        for (size_t i = 0u; i < count && i < header_count; ++i) {
            size_t width;

            if (markdown_table_cell_width(render, &cells[i], &width, &minimum[i]) < 0) return -1;
    ...
    if (wrapped) markdown_table_fit(widths, minimum, header_count, terminal_columns - 2u - 3u * header_count);
    if (markdown_inline_finish(render) < 0) return -1;
    ...
    size_t row = 0u;
    if (grid) {
        if (markdown_table_rule(render, "┌", "┬", "┐", widths, header_count, true) < 0 ||
            markdown_table_grid_row(render, header, header_count, widths,
                                    alignment, header_count, true) < 0 ||
            markdown_table_rule(render, "├", "┼", "┤", widths, header_count, true) < 0) return -1;
    ...
    while (markdown_table_next_line(text, md->table.len, &offset, &line, &line_len)) {
        ...
        if (render_checkpoint(render) < 0 || !markdown_table_cells(line, line_len, cells, &count)) return -1;
        if (grid) {
            if (row && wrapped && markdown_table_rule(render, "├", "┼", "┤", widths,
                                                       header_count, true) < 0) return -1;
            if (markdown_table_grid_row(render, cells, count, widths, alignment, header_count, false) < 0)
                return -1;
        } else {
            if (markdown_text(render, "├─ row\n", strlen("├─ row\n")) < 0) return -1;
            for (size_t i = 0u; i < header_count; ++i) {
                struct markdown_table_cell empty = {0};
                const struct markdown_table_cell *cell = i < count ? &cells[i] : &empty;

                if (markdown_table_vertical_cell(render, cell, &header[i], terminal_columns) < 0) return -1;
            }
        }
        ++row;
    }
    if (grid) {
        if (markdown_table_rule(render, "└", "┴", "┘", widths, header_count, ended_lf) < 0) return -1;
    } else {
        if (!row) {
            for (size_t i = 0u; i < header_count; ++i)
                if (markdown_table_vertical_cell(render, &header[i], NULL, terminal_columns) < 0) return -1;
        }
...
static int
markdown_table_admit(struct snag_render *render, const struct markdown_table_cell *cells, size_t count)
{
    struct snag_markdown_state *md = &render->markdown_state;
    ...
    if (!md->table_header_len) {
        ...
        if (!markdown_table_cells(md->table.data, header_len, header, &header_count) ||
            header_count != count || !markdown_table_delimiter(cells, count, alignment))
...
markdown_table_finish(struct snag_render *render)
{
    ...
    if (md->table_line) {
        ...
        md->table_line = false;
        if (!markdown_table_cells(md->table.data + md->table_line_start, md->table.len - md->table_line_start,
                                  cells, &count)) {
            if (md->table_active) {
                struct snag_buf row = {.max = SNAG_MAX_PUBLIC_ITEM};
                if (snag_buf_append(&row, md->table.data + md->table_line_start,
        ...
        if (markdown_table_admit(render, cells, count) < 0) return -1;
    }
    if (md->table_active) {
        rc = markdown_table_render(render);
        markdown_table_reset(md);
    ...
    if (md->table.len) return markdown_table_replay(render);
    return 0;
}
...
markdown_table_line_end(struct snag_render *render)
{
    ...
    md->table_line = false;
    md->line_start = true;
    if (!markdown_table_cells(md->table.data + md->table_line_start, line_len, cells, &count)) {
        md->table_line = true;
...
markdown_prefix_spaces(const struct snag_markdown_state *md)
{
    ...
    while (spaces < md->prefix_len && spaces < 3u && md->prefix[spaces] == ' ') ++spaces;
    return spaces;
}
...
markdown_prefix_literal(struct snag_render *render)
{
    ...
    md->line_start = false;
    if (prose && !md->prose) {
        if (markdown_paragraph(render, true) < 0) return -1;
        if (render->markdown_prose_bullets && markdown_text(render, "• ", strlen("• ")) < 0) return -1;
    }
    if (md->line_continuation && md->prose && render->markdown_prose_bullets &&
        markdown_text(render, "  ", 2u) < 0) return -1;
    md->line_continuation = false;
...
markdown_fence_close(const struct snag_markdown_state *md)
{
    ...
    while (i < md->prefix_len && md->prefix[i] == ' ') ++i;
...
markdown_open_fence(struct snag_render *render, bool newline)
{
    ...
    while (begin < end && md->fence_info[begin] == ' ') ++begin;
    while (end > begin && md->fence_info[end - 1u] == ' ') --end;
    if ((end - begin == 4u && memcmp(md->fence_info + begin, "text", 4u) == 0) ||
        (end - begin == 3u && memcmp(md->fence_info + begin, "txt", 3u) == 0) ||
        (end - begin == 9u && memcmp(md->fence_info + begin, "plaintext", 9u) == 0)) end = begin;
    md->fence_header = false;
    md->fence_info_len = 0u;
    if (markdown_text(render, "┌─", strlen("┌─")) < 0 || (end > begin &&
...
static int
markdown_line_prefix(struct snag_render *render, const unsigned char *text, size_t len)
{
    struct snag_markdown_state *md = &render->markdown_state;
    size_t spaces;
    const char *body;
    ...
    if (len != 1u || md->prefix_len == sizeof(md->prefix)) {
        if (md->fence) return markdown_code_prefix_literal(render) < 0 ? -1 :
    ...
    if ((md->table_pending || md->table_active) && !(body_len && body[0] == '|')) {
        size_t pending = md->prefix_len;
        char prefix[sizeof(md->prefix)];

        if (spaces == md->prefix_len && spaces <= 3u) return 0;
        memcpy(prefix, md->prefix, pending);
        md->prefix_len = 0u;
        if (markdown_table_finish(render) < 0) return -1;
        md->line_start = true;
        for (size_t i = 0u; i < pending; ++i)
            if (markdown_line_prefix(render, (const unsigned char *)prefix + i, 1u) < 0) return -1;
        return 0;
    }
    if (spaces == md->prefix_len) return spaces <= 3u ? 0 :
            (md->fence ? markdown_code_prefix_literal(render) : markdown_prefix_literal(render));
    if (md->fence) {
        size_t marks = 0u;
        while (marks < body_len && body[marks] == md->fence) ++marks;
        if (marks == body_len || (marks >= md->fence_len && body[marks] == ' ')) return 0;
        return markdown_code_prefix_literal(render);
    }
    if (!md->table_disabled && body[0] == '|') return markdown_table_start_line(render);
    if (body[0] == '#' && body_len >= 2u && body_len <= 7u && body[body_len - 1u] == ' ') {
        for (size_t i = 0u; i + 1u < body_len; ++i)
            if (body[i] != '#') goto ordinary;
        md->prefix_len = 0u;
        md->line_start = false;
        if (markdown_paragraph(render, false) < 0) return -1;
        md->heading = true;
    ...
    if ((body[0] == '`' || body[0] == '~')) {
        size_t marks = 0u;
        while (marks < body_len && body[marks] == body[0]) ++marks;
        if (marks == body_len && marks < 3u) return 0;
        if (marks >= 3u) {
            md->fence = body[0];
    ...
    if ((body[0] == '>' || body[0] == '-' || body[0] == '*' || body[0] == '+') && body_len == 1u) return 0;
    if (body_len == 2u && body[1] == ' ' && (body[0] == '>' || body[0] == '-' || body[0] == '*' ||
         body[0] == '+')) {
        const char *marker = body[0] == '>' ? "│ " : "• ";
        if (body[0] == '>') md->quote = true;
        md->prefix_len = 0u;
        md->line_start = false;
        if (markdown_paragraph(render, false) < 0) return -1;
        if (markdown_text(render, md->prefix, spaces) < 0) return -1;
        return markdown_text(render, marker, strlen(marker));
    }
    if (body[0] >= '0' && body[0] <= '9') {
        size_t digits = 0u;
        while (digits < body_len && body[digits] >= '0' && body[digits] <= '9') ++digits;
        if (digits == body_len && digits <= 9u) return 0;
        if (digits && digits <= 9u && digits + 1u == body_len && (body[digits] == '.' || body[digits] == ')'))
            return 0;
        if (digits && digits <= 9u && digits + 2u == body_len &&
            (body[digits] == '.' || body[digits] == ')') && body[digits + 1u] == ' ') {
...
ordinary: return markdown_prefix_literal(render);
}
...
markdown_newline(struct snag_render *render)
{
    ...
    if (md->fence_header) return markdown_open_fence(render, true);
    if (md->line_start) {
        ...
        if ((md->fence ? markdown_code_prefix_literal(render) : markdown_prefix_literal(render)) < 0)
            return -1;
    }
    if (markdown_inline_tail(render) < 0) return -1;
    if (markdown_text(render, "\n", 1u) < 0) return -1;
...
markdown_write(struct snag_render *render, const unsigned char *text, size_t len)
{
    ...
    while (i < len) {
        ...
        if (text[i] == '\n') {
            ...
            if (snag_buf_append(&md->table, text + i, n) < 0) return -1;
        } else if (md->fence_header) {
            if (n == 1u && text[i] == (unsigned char)md->fence && md->fence_info_len == 0u) {
                if (md->fence_len == UINT_MAX) return snag_errno(EOVERFLOW);
                ++md->fence_len;
                i += n;
                continue;
            }
            ...
            if (markdown_line_prefix(render, text + i, n) < 0) return -1;
        } else {
            size_t start = i;

            while (i + n < len && n < 1024u && text[i + n] != '\n') {
                size_t next = snag_utf8_size(text[i + n]);
                if (!next || next > len - i - n) break;
                n += next;
            }
            if (md->fence ? markdown_text(render, text + start, n) < 0 :
                            markdown_inline(render, text + start, n) < 0) return -1;
            i += n;
            continue;
        }
        i += n;
...
markdown_finish(struct snag_render *render)
{
    ...
    if (!render->markdown_rendering) return 0;
    if ((md->table_line || md->table_pending || md->table_active) && markdown_table_finish(render) < 0)
        return -1;
    if (md->fence_header) {
        if (markdown_open_fence(render, false) < 0) return -1;
        ...
        if (md->fence && markdown_fence_close(md)) {
            ...
            md->fence = '\0';
            md->fence_len = 0u;
        } else if ((md->fence ? markdown_code_prefix_literal(render) : markdown_prefix_literal(render)) < 0) {
            return -1;
        }
    }
    if (markdown_inline_tail(render) < 0) return -1;
    if (md->fence && !render->markdown_preserve_fence) {
        if (!render->public_item_ended_lf && markdown_text(render, "\n", 1u) < 0) return -1;
        if (markdown_text(render, "└─", strlen("└─")) < 0) return -1;
        md->fence = '\0';
        md->fence_len = 0u;
    }
...
{
    if (!state->pending.max) snag_buf_init(&state->pending, SNAG_CITE_BLOCK_MAX + 8u);
}
...
cite_turns(const char *body, size_t len, unsigned long *turns)
{
    ...
    while (pos < len) {
        ...
        if (len - pos < 3u || memcmp(body + pos, SNAG_CITE_SEP, 3u) || count == SNAG_CITE_MAX_TURNS ||
            len - pos - 3u < 4u || memcmp(body + pos + 3u, "turn", 4u)) return 0u;
        pos += 7u;
        while (pos + digits < len && digits < 9u && body[pos + digits] >= '0' && body[pos + digits] <= '9') {
            value = value * 10ul + (unsigned long)(body[pos + digits] - '0');
            ++digits;
        }
        if (!digits) return 0u;
        pos += digits;
        if (len - pos >= 4u && !memcmp(body + pos, "view", 4u)) pos += 4u;
        else if (len - pos >= 6u && !memcmp(body + pos, "search", 6u)) pos += 6u;
        else return 0u;
        digits = 0u;
        while (pos + digits < len && digits < 9u && body[pos + digits] >= '0' && body[pos + digits] <= '9')
            ++digits;
...
cite_rewrite(const char *block, size_t len, struct snag_buf *out)
{
    ...
    if (len < 23u || memcmp(block, SNAG_CITE_OPEN, 3u) || memcmp(block + len - 3u, SNAG_CITE_CLOSE, 3u) ||
        memcmp(block + 3u, "cite", 4u)) return 0;
    count = cite_turns(block + 7u, len - 10u, turns);
    if (!count) return 0;
    for (size_t i = 0u; i < count; ++i) {
        size_t slot = 0u;

        while (slot < total && unique[slot] != turns[i]) ++slot;
        if (slot < total) continue;
        if (total == SNAG_CITE_MAX_TURNS) return 0;
        slot = total;
    ...
    if (snag_buf_append(out, "[cite: turn ", sizeof("[cite: turn ") - 1u) < 0) return -1;
    for (size_t i = 0u; i < total;) {
        char part[48];
        int written;
        size_t run = i;

        while (run + 1u < total && unique[run + 1u] == unique[run] + 1ul) ++run;
        written = run > i ? snprintf(part, sizeof(part), "%lu-%lu", unique[i], unique[run]) :
            snprintf(part, sizeof(part), "%lu", unique[i]);
        if (written < 0 || (size_t)written >= sizeof(part)) return 0;
        if (i && snag_buf_append(out, ", ", 2u) < 0) return -1;
        if (snag_buf_append(out, part, (size_t)written) < 0) return -1;
        i = run + 1u;
    }
...
cite_feed(struct snag_cite_state *state, const char *text, size_t len, struct snag_buf *out)
{
    ...
    while (pos < len) {
        ...
        scan = pos;
        while (scan + 3u <= len && memcmp(text + scan, SNAG_CITE_CLOSE, 3u)) ++scan;
        if (scan + 3u > len) {
...
cite_flush(struct snag_cite_state *state, struct snag_buf *out)
{
    ...
    if (state->active) rc = snag_buf_append(out, state->pending.data, state->pending.len);
    snag_buf_reset(&state->pending);
    state->active = false;
    return rc;
}
              ...
              const char *text, size_t len, struct snag_buf *complete)
{
    for (size_t i = 0u; i < len; ++i) {
        if (*pending_len >= 4u) goto invalid;
        pending[(*pending_len)++] = (unsigned char)text[i];
        size_t expected = snag_utf8_size(pending[0]);
        if (!expected || *pending_len > expected) goto invalid;
        if (*pending_len < expected) continue;
        if (!snag_utf8_valid(pending, expected, true)) goto invalid;
        if (snag_buf_append(complete, pending, expected) < 0) return -1;
        *pending_len = 0u;
    }
    return 0;
invalid: *pending_len = 0u;
    return snag_errno(EILSEQ);
}

static int
public_emit(struct snag_render *render, const unsigned char *data, size_t len, struct snag_buf *delivered)
{
    if (!len) return 0;
    if (public_terminal(render) && render->boundary == BOUNDARY_PROMPT &&
        !render->wrap_pending.len && markdown_gap(render) < 0) return -1;
    if (delivered && snag_buf_reserve(delivered, len) < 0) return -1;
    if (public_terminal(render) ? (render->markdown_rendering ?
         markdown_write(render, data, len) < 0 : write_wrapped(render, data, len) < 0) :
        public_write(render, (const char *)data, len, 0u) < 0) return -1;
    if (delivered && snag_buf_append(delivered, data, len) < 0) return -1;
    return 0;
}
...
int
snag_render_public(struct snag_render *render, const char *text, size_t len, struct snag_buf *delivered)
{
    if (!len) return render_public_chunk(render, text, len, delivered);
    while (len) {
        size_t amount = text_slice(text, len);
        if (render_public_chunk(render, text, amount, delivered) < 0) return -1;
        text += amount;
        len -= amount;
...
close_public_item(struct snag_render *render, bool discard_incomplete)
{
    ...
    if (!render->public_item_open) {
        if (invalid) return snag_errno(EILSEQ);
        return 0;
    }
    ...
    if (render->markdown_rendering && markdown_paragraph(render, false) < 0 && rc == 0) {
        rc = -1;
        saved_errno = errno;
    }
    ...
    if (had_bytes && !ended_lf && (fd == STDERR_FILENO || render->stderr_terminal) &&
        write_block(render, STDERR_FILENO, "\n", 1u, false, true) < 0) rc = -1;
    ...
    if (rc < 0 && !saved_errno) saved_errno = errno;
    if (saved_errno) errno = saved_errno;
    if (invalid && rc == 0) {
        errno = EILSEQ;
        rc = -1;
...
static int
rollout_physical_begin(struct snag_render *render, struct snag_render_record *record)
{
    const char *label;

    if (record->physical_open) return 0;
    label = record->label_displayed ? NULL : record->label;
    if (snag_render_public_begin(render, record->fd, label) < 0) return -1;
...
int
snag_render_rollout_begin(struct snag_render *render, int fd, const char *label, enum snag_presentation kind)
{
    struct snag_render_record *record;

    if (render->rollout_open || render->public_item_open || (fd != STDOUT_FILENO && fd != STDERR_FILENO)) {
...
int
snag_render_rollout(struct snag_render *render, const char *text, size_t len, struct snag_buf *delivered)
{
...
close_rollout_record(struct snag_render *render, bool abort)
{
    ...
    if (record->physical_open) {
        if ((abort ? snag_render_public_abort(render) : snag_render_public_end(render)) < 0 && rc == 0)
            rc = -1;
        record->physical_open = false;
    }
    render->rollout_open = NULL;
...
static int
render_message(struct snag_render *render, const char *message, const char *color)
{
    int rc;
...
{
    size_t len = strlen(text);
    if (close_public_output(render) < 0) return -1;
    int rc = write_banner(render, BOUNDARY_UPDATE, COLOR_UPDATE, text, len, first_line_len(text, len));
    if (render->stderr_terminal && render->public_item_open) render->public_column = 0u;
    return rc;
}
...
snag_render_host(struct snag_render *render, const char *text)
{
    ...
    if (rc == 0) rc = write_role_block(render, BOUNDARY_CONTENT, STDERR_FILENO, COLOR_HOST,
...
snag_render_runtime(struct snag_render *render, const char *text)
{
    ...
    if (!snag_render_enabled(render, SNAG_PRESENT_DEBUG)) return 0;
    len = strlen(text);
    struct snag_buf line = {.max = 4u * 1024u * 1024u};
    rc = snag_buf_append(&line, text, len);
    if (rc == 0 && (!len || text[len - 1u] != '\n')) rc = snag_buf_putc(&line, '\n');
    if (rc == 0) rc = write_optional_block(render, SNAG_PRESENT_DEBUG, COLOR_META,
                        (char *)line.data, line.len, first_line_len((char *)line.data, line.len));
    snag_buf_free(&line);
    return rc;
}
...
irc_markdown_state(struct snag_render *render, const struct snag_irc_event *event, bool allocate)
{
...
static void
irc_markdown_lifecycle(struct snag_render *render, const struct snag_irc_event *event)
{
    bool endpoint_reset = event->kind == SNAG_IRC_CONNECTED || event->kind == SNAG_IRC_DISCONNECTED;
    bool nick_reset = event->kind == SNAG_IRC_PART || event->kind == SNAG_IRC_QUIT ||
...
render_irc_markdown(struct snag_render *render, const struct snag_irc_event *event, size_t column)
{
    ...
    if (snag_render_public_begin(&body, STDERR_FILENO, NULL) < 0) return -1;
    ...
    return rc;
out: body.markdown_rendering = false;
...
int
snag_render_irc_event(struct snag_render *render, const struct snag_irc_event *event)
{
    struct snag_render_record *record;
    ...
    while (render->view_head[view]) {
        ...
        if (record->kind == SNAG_RENDER_RECORD_BLOCK) {
            rc = write_role_block(render, record->boundary, record->fd, record->color,
                                  (const char *)record->text.data, record->text.len, record->colored_len,
            ...
            if (rc == 0 && record->displayed < record->text.len) rc = rollout_physical_append(
                    render, record, (const char *)record->text.data + record->displayed,
...
tool_body(struct snag_render *render, const struct snag_render_block *block)
{
    ...
    if (!block->body.len || !snag_render_enabled(render, block->body_kind)) return 0;
    char header[32];
    (void)snprintf(header, sizeof(header), "  %s:\n", label);
    if (write_block(render, STDERR_FILENO, header, strlen(header), true, true) < 0) return -1;
    while (offset < block->body.len) {
        ...
        while (end < block->body.len && end - offset < 1024u && count + characters < limit) {
            ...
            if (!n || n > block->body.len - end) return snag_errno(EILSEQ);
            end += n;
            ++characters;
        }
        if (end == offset) {
        ...
        if (write_block(render, STDERR_FILENO, (const char *)block->body.data + offset,
                         end - offset, true, true) < 0) return -1;
        offset = end;
        count += characters;
        if (render_checkpoint(render) < 0) return -1;
    }
...
snag_render_tool_block(struct snag_render *render, const struct snag_render_block *block)
{
    ...
    if (write_role_block(render, BOUNDARY_CONTENT, STDERR_FILENO,
                      ...
                      render->stderr_terminal, true) < 0 || render_checkpoint(render) < 0) goto out;
    if (block->context.len && snag_render_enabled(render, SNAG_PRESENT_CONTEXT) &&
        write_optional_block(render, SNAG_PRESENT_CONTEXT, "", (const char *)block->context.data,
                              block->context.len, 0u) < 0) goto out;
...
source_event(struct snag_render *render, struct snag_render_source source)
{
    ...
    if (snag_buf_reserve(&text, source.len) < 0) goto out;
                       ...
                       uint32_t max_output_bytes, int64_t result_offset)
{
    ...
    while (start < end && snag_render_enabled(render, SNAG_PRESENT_OUTPUT)) {
        ...
        for (ssize_t i = 0; i < n; ++i) {
            if (input[i] != '\n') {
                if (snag_buf_putc(&line, input[i]) < 0) goto out;
                continue;
            }
            char error[128];
...
render_tool_record(struct snag_render *render, const struct snag_render_record *record)
{
    ...
    for (size_t i = 0u; id && i < json_array_size(items); ++i) {
        ...
        if (!call_id || !name || strcmp(call_id, id) != 0) continue;
        unsigned int columns = render->term ? render->term->columns : 0u;
        if (record->tool_start) {
            struct snag_response_item call = {
                .name = (char *)name, .arguments = json_object_get(item, "arguments"), .call_id = call_id };
            ...
            rc = snag_render_prepare_tool_finish(&block, name, call_id, json_object_get(data, "result"),
                    ...
                    const char *type, uint32_t timeout_ms, uint32_t max_output_bytes)
{
    ...
    if (strcmp(type, "goal_lock_changed") == 0 || strcmp(type, "goal_paused") == 0) {
        ...
        if (strcmp(type, "goal_paused") == 0) {
            ...
            notice = reason && strcmp(reason, "refusal") == 0 ? "Goal paused after model refusal" :
    ...
    if (snag_string_in(type, "response_completed response_interrupted response_failed")) {
        ...
        for (struct snag_render_record *record = render->view_head[SNAG_RENDER_ROLLOUT];
            ...
            if (record->kind != SNAG_RENDER_RECORD_PUBLIC || record->source.len || !record->complete)
                continue;
            ...
            for (size_t i = 0u; i < json_array_size(items); ++i) {
                ...
                if (!json_is_string(text) || json_string_length(text) != record->text.len ||
                    memcmp(json_string_value(text), record->text.data, record->text.len) != 0) continue;
                record->source = source;
...
snag_render_event(struct snag_render *render, uint64_t seq, const char *type)
{
    ...
    if (notice) rc = render_bullet(render, notice);
    snag_buf_reset(&line);
    if (rc == 0 && debug) {
        rc = snag_buf_printf(&line, "event › %llu %s synced\n", (unsigned long long)seq, type);
        if (rc == 0) rc = write_optional_block(render, SNAG_PRESENT_DEBUG, COLOR_DURABLE,
                                       (char *)line.data, line.len, line.len);
    }
    snag_buf_free(&line);
    return rc;
...
snag_render_resume_hint(const struct snag_render *render, const char *command, size_t command_len)
{
    ...
    if (colored && snag_buf_append(&block, COLOR_LIFECYCLE, sizeof(COLOR_LIFECYCLE) - 1u) < 0) goto out;
    if (snag_buf_append(&block, header, sizeof(header) - 1u) < 0 ||
        snag_buf_append(&block, note, strlen(note)) < 0 || snag_buf_putc(&block, ':') < 0) goto out;
...
{
    if (!text || !snag_utf8_valid((const unsigned char *)text, len, true)) return false;
...
{
    static const char warning[] = SNAJPAGENT_NAME
        ": verbosity 5 exposes prompts, source code, tool output, and model content\n";

    if (render->protocol_warning_shown) return 0;
    if (write_role_block(render, BOUNDARY_CONTENT, STDERR_FILENO, COLOR_WARNING,
                         warning, sizeof(warning) - 1u, sizeof(warning) - 1u,
                         render->stderr_terminal, true) < 0) return -1;
    render->protocol_warning_shown = true;
    return 0;
}

int
snag_render_protocol(struct snag_render *render, const char *label, const char *text, size_t len)
{
    int rc = -1;

    if (!snag_render_enabled(render, SNAG_PRESENT_PROTOCOL)) return 0;
    if (!label || !diagnostic_text_valid(label, strlen(label), false) ||
        !diagnostic_text_valid(text, len, true) || len > 2u * 1024u * 1024u) return snag_errno(EINVAL);
    if (protocol_warning(render) < 0) return -1;
    struct snag_buf block = {.max = 2u * 1024u * 1024u + 4096u};
    if (snag_buf_printf(&block, "protocol › %s\n", label) < 0 || snag_buf_append(&block, text, len) < 0 ||
        (len && text[len - 1u] != '\n' && snag_buf_putc(&block, '\n') < 0)) goto out;
    rc = write_optional_block(render, SNAG_PRESENT_PROTOCOL, COLOR_PROTOCOL,
                    (const char *)block.data, block.len, first_line_len((const char *)block.data, block.len));
out: snag_buf_free(&block);
    return rc;
}

int
snag_render_transport(struct snag_render *render, char direction, const char *text, size_t len)
{
    int rc = -1;

    if (!snag_render_enabled(render, SNAG_PRESENT_WIRE)) return 0;
    if ((direction != '>' && direction != '<') ||
        !diagnostic_text_valid(text, len, false) || len > 64u * 1024u) return snag_errno(EINVAL);
    if (protocol_warning(render) < 0) return -1;
    struct snag_buf line = {.max = 64u * 1024u + 4u};
    if (snag_buf_putc(&line, (unsigned char)direction) < 0 || snag_buf_putc(&line, ' ') < 0 ||
        snag_buf_append(&line, text, len) < 0 || snag_buf_putc(&line, '\n') < 0) goto out;
    rc = write_optional_block(render, SNAG_PRESENT_WIRE, COLOR_TRANSPORT, (const char *)line.data, line.len,
