...
open_entry(int parent, const char *name)
{
    ...
    if (!S_ISREG(before.st_mode) && !S_ISDIR(before.st_mode)) return snag_errno(EINVAL);
    fd = snag_open_read_at(parent, name, false);
    if (fd < 0) return -1;
    if (snag_fstat(fd, &after) < 0 || before.st_dev != after.st_dev || before.st_ino != after.st_ino ||
...
open_path(const char *workspace, const char *path)
{
    ...
    if (snag_buf_printf(&full, "%s%s%s", absolute ? "" : workspace, absolute ? "" : "/", path) < 0 ||
        snag_buf_terminate(&full) < 0) goto out;
    copy = (char *)full.data;
...
literal_match(const char *line, const char *pattern, bool ignore_case)
{
    ...
    do {
        size_t i = 0;
        while (pattern[i] && line[i] && tolower((unsigned char)line[i]) ==
               tolower((unsigned char)pattern[i])) ++i;
...
static int
emit_line(struct read_query *q, const char *path, uint64_t line, const char *text)
{
    size_t needed = strlen(path) + strlen(text) + 48u;

    if (q->seen++ < q->offset) return 0;
    if (q->emitted == q->limit || needed > RO_OUTPUT - q->output.len) {
        q->more = true;
        return 1;
    }
    if (snag_buf_printf(&q->output, "%s:%llu:%s\n", path, (unsigned long long)line, text) < 0) return -1;
    ++q->emitted;
    return 0;
}
...
scan_file(struct read_query *q, int fd, const char *path)
{
    ...
    for (;;) {
        ...
        if (c == EOF && !len) break;
        if (memchr(line, 0, len) || (len < RO_LINE && !snag_utf8_valid((unsigned char *)line, len, true))) {
            if (q->grep) {
...
walk(struct read_query *q, int fd, const char *path, unsigned int depth)
{
    ...
    if (S_ISREG(st.st_mode) && (q->grep || q->read)) return scan_file(q, fd, path);
    ...
    while (checkpoint(q) == 0) {
        ...
        if (snag_string_in(entry, ". ..")) continue;
        if (++q->entries > RO_ENTRIES) {
            q->problem = "Directory entry limit reached; narrow the path.";
            break;
        }
        if (!snag_utf8_valid((unsigned char *)entry, strlen(entry), true)) {
    ...
    for (size_t i = 0; i < count && rc == 0; ++i) {
        ...
        if (checkpoint(q) < 0 || snag_lstat_at(fd, names[i], &st) < 0) {
            rc = -1;
            break;
        }
        struct snag_buf child = {.max = 8192u};
        rc = snag_buf_printf(&child, "%s%s%s", path, path[strlen(path) - 1u] == '/' ? "" : "/", names[i]);
        if (rc == 0) rc = snag_buf_terminate(&child);
        type = S_ISDIR(st.st_mode) ? "directory" : S_ISREG(st.st_mode) ? "file" :
               S_ISLNK(st.st_mode) ? "symlink (not followed)" : "special (not opened)";
        if (q->grep && !S_ISDIR(st.st_mode) && !S_ISREG(st.st_mode)) ++q->skipped;
        if (rc == 0 && !q->grep) {
            if (q->seen++ >= q->offset) {
                if (q->emitted == q->limit || child.len + strlen(type) + 4u > RO_OUTPUT - q->output.len) {
                    q->more = true;
                    rc = 1;
                } else {
        ...
        if (rc == 0 && ((S_ISDIR(st.st_mode) && q->recursive) || (S_ISREG(st.st_mode) && q->grep))) {
            int next;
                    ...
                    snag_tool_pump_fn pump, void *opaque, json_t **result)
{
    ...
    *result = NULL;
    ...
    q.problem = failure;
    if (!snag_read_only_tool(call->name) || !snag_json_arg_keys(args, q.grep ? "path pattern" : "path",
            q.read ? "start_line end_line" : q.grep ? "recursive ignore_case literal offset limit" :
                     "recursive offset limit", failure, sizeof(failure)) ||
        !snag_json_arg_text(args, "path", 1u, 4096u, false, &path, failure, sizeof(failure))) goto out;
    if (q.read) {
    ...
    if (q.grep) {
        if (!snag_json_arg_text(args, "pattern", 0u, 4096u, false, &q.pattern, failure, sizeof(failure)) ||
            ...
            !snag_json_arg_bool(args, "literal", false, &q.literal, failure, sizeof(failure))) goto out;
        if (!q.literal) {
