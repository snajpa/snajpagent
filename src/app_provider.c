...
enum snag_managed_continuation
snag_app_managed_continuation_classify(const struct app_state *app,
                                      const struct snag_response_graph *graph,
                                      const struct snag_graph_decision *decision)
{
    ...
    if (!handle[0])
        return SNAG_MANAGED_CONTINUATION_NONE;
    if (decision->outcome != SNAG_GRAPH_CALLS)
        return SNAG_MANAGED_CONTINUATION_ORDERING_VIOLATION;
    for (size_t i = 0; i < graph->count; ++i) {
        ...
        if (call->name && strcmp(call->name, "write_stdin") == 0) {
            if (continuation)
                return SNAG_MANAGED_CONTINUATION_ORDERING_VIOLATION;
            continuation = true;
            arg_handle = snag_json_string(call->arguments, "handle");
    ...
    if (!continuation)
        return SNAG_MANAGED_CONTINUATION_ORDERING_VIOLATION;
    return mismatch ? SNAG_MANAGED_CONTINUATION_HANDLE_MISMATCH :
                      SNAG_MANAGED_CONTINUATION_MATCHED;
}
