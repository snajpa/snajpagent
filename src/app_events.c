...
turn_config(const struct app_state *app)
{
    ...
    if (!config ||
        snag_json_set_new(config, "max_parallel_commands",
                         json_integer(app->config->max_parallel_commands)) < 0 ||
        snag_json_set_new(config, "parallel_tool_calls",
                     ...
                     uint64_t offset, const void *bytes, size_t len)
{
    ...
    if (!event || (!utf8 && snag_base64_append(&encoded, bytes, len) < 0) ||
        snag_json_set_new(event, "turn_id", json_string(app->session.active_turn_id)) < 0 ||
        snag_json_set_new(event, "handle", json_string(handle)) < 0 ||
        snag_json_set_new(event, "stream", json_integer(stream)) < 0 ||
        snag_json_set_new(event, "offset", json_integer((json_int_t)offset)) < 0 ||
        snag_json_set_new(event, "encoding", json_string(utf8 ? "utf8" : "base64")) < 0 ||
        snag_json_set_new(event, "data", json_stringn(utf8 ? bytes : (const void *)encoded.data,
