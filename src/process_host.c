...
peer_transfer(HANDLE handle, bool write, void *bytes, DWORD size, bool asynchronous)
{
    ...
    if (io.hEvent) (void)CloseHandle(io.hEvent);
    SetLastError(error);
    return !error;
}
...
verify_pipe_pair(struct child_pipe *pipe, bool input, HANDLE other, bool asynchronous)
{
    ...
    if (difference) return snag_errno(EACCES);
    (void)ResetEvent(pipe->io.hEvent);
    pipe->ready = false;
    pipe->count = 0;
    return 0;
...
create_pipe(struct child_pipe *pipe, bool input, HANDLE *other, bool asynchronous)
{
    ...
    if (swprintf(name, 96u, L"\\\\.\\pipe\\snajpagent-%hs", id) < 0) return -1;
    ...
    if (!pipe->io.hEvent) return child_error(GetLastError());
...
{
    if (broker && broker->process) (void)TerminateProcess(broker->process, 125u);
}
...
snag_output_broker_close(struct snag_output_broker *broker)
{
    ...
    if (broker->control.io.hEvent) (void)CloseHandle(broker->control.io.hEvent);
    if (broker->process) {
...
broker_wait(struct snag_output_broker *broker, uint64_t deadline, int (*checkpoint)(void *), void *opaque)
{
    ...
    while (!pipe_done(pipe)) {
        ...
        if (rc == WAIT_FAILED) return child_error(GetLastError());
        if (rc == WAIT_OBJECT_0 + 1u) return snag_errno(EPIPE);
        if (rc == WAIT_TIMEOUT) {
            if (checkpoint && checkpoint(opaque) < 0) return -1;
            if (deadline && snag_monotonic_ms() >= deadline) return snag_errno(ETIMEDOUT);
        }
    }
                ...
                int (*checkpoint)(void *), void *opaque)
{
    ...
    while (size) {
        ...
        if (write) memcpy(pipe->bytes, bytes, amount);
        pipe_begin(pipe, write, amount);
        if (broker_wait(broker, deadline, checkpoint, opaque) < 0) return -1;
        if (!pipe->count || pipe->count > amount) return snag_errno(EIO);
        if (!write) memcpy(bytes, pipe->bytes, pipe->count);
        volatile unsigned char *wipe = pipe->bytes;
        for (DWORD i = 0; i < pipe->count; ++i) wipe[i] = 0;
        bytes += pipe->count;
        size -= pipe->count;
    }
    return 0;
...
broker_open(int (*checkpoint)(void *), void *opaque, bool console)
{
    ...
    if (snag_random_id(id) < 0 || snag_random_bytes(nonce, sizeof(nonce)) < 0) goto fail;
    DWORD length = GetModuleFileNameW(NULL, program, 32768u);
    if (!length) goto native_error;
...
broker_parent_wait(void *parent)
{
    ...
    if (job) (void)TerminateJobObject(job, 125u);
    ExitProcess(125u);
    return 0;
}
...
broker_child_spawn(HANDLE pipe, HANDLE parent, bool pty)
{
    ...
    if (sent && !reply.error && ResumeThread(child.hThread) == (DWORD)-1) sent = false;
out:
    if (!sent || reply.error) (void)TerminateJobObject(job, 125u);
    if (child.hThread) (void)CloseHandle(child.hThread);
    if (child.hProcess) (void)CloseHandle(child.hProcess);
    for (size_t i = 0; i < 4u; ++i) {
        if (text[i]) {
            volatile wchar_t *wipe = text[i];
            for (size_t j = 0; j < request.units[i]; ++j) wipe[j] = 0;
        }
        free(text[i]);
    }
#ifdef SNAG_LEGACY_PTY
    ...
    for (size_t i = 0; i < 3u; ++i)
        if (streams[i]) (void)CloseHandle(streams[i]);
    for (size_t i = 0; i < 2u; ++i)
        if (console_streams[i] != INVALID_HANDLE_VALUE) (void)CloseHandle(console_streams[i]);
...
snag_output_broker_main(int argc, wchar_t **argv)
{
    ...
    for (;;) {
        ...
        if (!broker_child_transfer(pipe, false, &operation, sizeof(operation))) {
            if (GetLastError() == ERROR_BROKEN_PIPE || GetLastError() == ERROR_NO_DATA) result = 0;
            break;
        }
        ...
        if (operation == BROKER_READ_CONSOLE) {
            ...
            for (size_t i = 0; i < 256u; ++i) wipe[i] = 0;
            if (!sent) break;
            continue;
        }
        ...
        if (fd < 0) {
            ...
            if (snag_term_output_write(NULL, fd, bytes, (size_t)packet[1], false, NULL, NULL) < 0)
                status = errno;
            if (_close(fd) < 0 && !status) status = errno;
        }
        if (!broker_child_transfer(pipe, true, &status, sizeof(status))) break;
    }
out:
    for (size_t i = 0; i < 2u; ++i)
        if (standard[i]) (void)CloseHandle(standard[i]);
    if (input != INVALID_HANDLE_VALUE) (void)CloseHandle(input);
    {
        HANDLE job = atomic_load(&broker_owned_job);
        if (job) (void)TerminateJobObject(job, 125u);
    }
...
{
    if (!child->native || stream >= 3u || (child->pty && stream == 2u)) return;
    struct child_pipe *pipe = &child->native->pipe[stream];
    if (pipe->handle) {
        pipe_cancel(pipe);
        (void)CloseHandle(pipe->handle);
    }
    if (pipe->io.hEvent) (void)CloseHandle(pipe->io.hEvent);
...
snag_child_signal(struct snag_child *child, enum snag_child_signal signal)
{
    ...
    if (!native || !native->process || child->reaped) return;
...
snag_child_free(struct snag_child *child)
{
    struct snag_child_windows *native = child->native;
    if (!native) return;
    if (native->job) (void)TerminateJobObject(native->job, 137u);
            ...
            const char *directory, char **environment, bool pty, bool isolated)
{
    ...
    if (native->legacy_console) {
        ...
        if (native->dimensions.X > 2500) native->dimensions.X = 2500;
        if (native->dimensions.Y > 2000) native->dimensions.Y = 2000;
    }
#endif
    ...
    if (!_wcsicmp(base, L"cmd.exe") || !_wcsicmp(base, L"cmd")) {
        ...
        if (snag_buf_append(&line, " -c \"", 5u) < 0) goto out;
    ...
    if (snag_buf_terminate(&line) < 0 || !(text = snag_utf8_to_wide((char *)line.data))) goto out;
    ...
    if (!isolated && !SetInformationJobObject(native->job, JobObjectExtendedLimitInformation,
                                               &limits, sizeof(limits))) goto native_error;
    if (isolated) {
    ...
    if (!startup.lpAttributeList) goto out;
...
finish_console(struct snag_child_windows *native)
{
    ...
    if (!info.ActiveProcesses) {
        ...
        if (!native->console_closer) return -1;
    }
    return 0;
}
...
snag_child_resize(struct snag_child *child)
{
    ...
    if (native && native->legacy_console) {
        ...
        if (control->pending && !pipe_done(control)) return;
        if (control->error) {
            (void)TerminateJobObject(native->job, 125u);
            return;
        }
        COORD size = console_dimensions();
        if (size.X > 2500) size.X = 2500;
        if (size.Y > 2000) size.Y = 2000;
...
snag_child_write(struct snag_child *child, const void *buffer, size_t size)
{
    ...
    if (!pipe->handle) return snag_errno(EPIPE);
    if (!pipe->pending && !pipe->ready) {
        if (size > sizeof(pipe->bytes)) size = sizeof(pipe->bytes);
...
snag_child_wait(struct snag_child_event *events, size_t count, snag_wake_fd wake, int timeout_ms)
{
    ...
    int rc = 0;
    if (count > 96u || timeout_ms < -1) return snag_errno(EINVAL);
    if (wake != SNAG_WAKE_INVALID) {
        wake_event = WSACreateEvent();
        if (wake_event == WSA_INVALID_EVENT) return snag_socket_error(WSAGetLastError());
    ...
    for (;;) {
        ...
        if (wake_event) {
            if (WaitForSingleObject(wake_event, 0) == WAIT_OBJECT_0) ++rc;
...
{
    if (pid <= 0) return;
    if (kill(-pid, signo) < 0 && errno == ESRCH) (void)kill(pid, signo);
}
           ...
           int stdin_rd, int stdout_wr, int stderr_wr, char **env)
{
    ...
    if (dup2(stdin_rd, STDIN_FILENO) < 0 || dup2(stdout_wr, STDOUT_FILENO) < 0 ||
        dup2(stderr_wr, STDERR_FILENO) < 0) _exit(125);
...
pty_apply_current_size(int fd, unsigned short *rows, unsigned short *cols)
{
    ...
    if (*rows == new_rows && *cols == new_cols) return;
    memset(&ws, 0, sizeof(ws));
    ws.ws_row = new_rows;
    ws.ws_col = new_cols;
...
static int
open_pty_pair(int *master_fd, int *slave_fd, unsigned short *rows, unsigned short *cols)
{
    struct winsize ws;
...
proc_child_exited(struct snag_child *child)
{
    ...
    if (length < 0 || (size_t)length >= sizeof(path)) return snag_errno(EOVERFLOW);
    int fd = snag_open_read(path, false);
    if (fd < 0) {
        if (errno == ENOENT || errno == ESRCH) errno = ECHILD;
        return -1;
    }
    ssize_t n;
    do {
...
snag_child_exited(struct snag_child *child)
{
    ...
    if (sysctl(mib, 6u, &info, &size, NULL, 0) < 0) return -1;
...
#endif
    size_t size = 0;
    if (sysctl(mib, 3u, NULL, &size, NULL, 0) < 0) return -1;
    ...
    if (pid < 0 && errno == ECHILD) child->reaped = true;
...
snag_child_reap(struct snag_child *child)
{
    ...
    if (got != child->pid) {
        if (got < 0 && errno == ECHILD) child->reaped = true;
        return -1;
    }
    child->reaped = true;
...
snag_child_close_stream(struct snag_child *child, unsigned int stream)
{
    if (stream >= 3u || (child->pty && stream == 2u)) return;
    int fd = child->fd[stream];
    close_if_open(&child->fd[stream]);
    for (size_t i = 0; i < 3u; ++i)
        if (child->fd[i] == fd) child->fd[i] = -1;
}
...
snag_child_resize(struct snag_child *child)
{
    if (child->pty) pty_apply_current_size(child->fd[0], &child->rows, &child->columns);
}
...
snag_child_wait(struct snag_child_event *events, size_t count, snag_wake_fd wake, int timeout_ms)
{
    ...
    if (rc < 0) return rc;
    for (size_t i = 0; i < count; ++i) {
        if (fds[i].revents & POLLIN) events[i].revents |= SNAG_CHILD_READ;
        if (fds[i].revents & POLLOUT) events[i].revents |= SNAG_CHILD_WRITE;
        if (fds[i].revents & (POLLHUP | POLLERR)) events[i].revents |= SNAG_CHILD_END;
        if (fds[i].revents & POLLNVAL) events[i].revents |= SNAG_CHILD_ERROR;
    }
    return rc;
}
#endif
