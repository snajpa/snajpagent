...
snag_provider_failure_retryable(long status, const char *code, const char *type)
{
    ...
    for (size_t k = 0; k < sizeof(kinds) / sizeof(kinds[0]); ++k) {
        ...
        for (i = 0; i < sizeof(transient) / sizeof(transient[0]); ++i)
            if (strcmp(kinds[k], transient[i]) == 0) break;
        if (i == sizeof(transient) / sizeof(transient[0])) return false;
    }
    return true;
}

int
snag_provider_retry_after_parse(const unsigned char *value, size_t len, uint32_t *delay_ms)
{
    uint64_t seconds = 0u;
    size_t i = 0u;
    size_t first;

    if (delay_ms) *delay_ms = 0u;
    if (!value || !delay_ms) return -1;
    while (i < len && (value[i] == ' ' || value[i] == '\t')) ++i;
    first = i;
    while (i < len && value[i] >= '0' && value[i] <= '9') {
        uint64_t digit = (uint64_t)(value[i] - '0');
        if (seconds > (UINT64_MAX - digit) / 10u) return -1;
        seconds = seconds * 10u + digit;
        ++i;
    }
    if (i == first) return -1;
    while (i < len && (value[i] == ' ' || value[i] == '\t')) ++i;
    if (i != len || seconds > SNAG_PROVIDER_RETRY_AFTER_MAX_MS / 1000u) return -1;
