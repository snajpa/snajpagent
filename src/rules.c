...
{
    if (!name || !*name || strlen(name) > SNAG_RULE_NAME_MAX)
        return false;
...
{
    return boundary && (!strcmp(boundary, "in") || !strcmp(boundary, "out") ||
                        !strcmp(boundary, "event"));
}
...
{
    if (!name_valid(name) || chain_index(rules, name) != rules->chain_count)
        return -1;
    if (rules->chain_count >= SNAG_CHAINS_MAX)
        return -1;
    (void)snprintf(rules->chains[rules->chain_count], SNAG_RULE_NAME_MAX + 1u,
                   "%s", name);
    ++rules->chain_count;
    return 0;
}
...
append_text(struct snag_buf *out, const json_t *value)
{
    if (json_is_string(value))
        return snag_buf_append(out, json_string_value(value),
...
snag_rule_render(const char *format, const json_t *envelope, struct snag_buf *out)
{
    ...
    for (const char *c = format; *c; ++c) {
        if (*c != '%') {
            if (snag_buf_putc(out, (unsigned char)*c) < 0)
                return -1;
            continue;
        }
        if (c[1] == '%') {
        ...
        if (c[1] != '{') {
            if (snag_buf_putc(out, (unsigned char)'%') < 0)
                return -1;
            continue;
        }
        ...
        if (len == 0u || len >= sizeof(path))
            return snag_errno(EINVAL);
        memcpy(path, c + 2, len);
        path[len] = '\0';
        if (!pointer_valid(path))
            return snag_errno(EINVAL);
        const json_t *value = pointer_get(envelope, path);
        if (value && append_text(out, value) < 0)
            return -1;
        c = end;
    }
    return 0;
...
rule_free(struct snag_rule *rule)
{
    ...
    for (size_t i = 0u; i < rule->threshold_count; ++i)
        free(rule->thresholds[i].pointer);
...
verb_parse(const char *name)
{
    ...
    for (size_t i = 0u; i < SNAG_RULE_VERB_COUNT; ++i)
        if (!strcmp(name, names[i]))
            return (enum snag_rule_verb)i;
    return SNAG_RULE_VERB_COUNT;
}

static int
compile_rule(struct snag_rules *rules, json_t *definition, size_t index,
             char *error, size_t size)
{
    ...
    if (!json_is_object(definition) || !name_valid(name) || !name_valid(chain) ||
        !action || !*action)
        ...
        for (void *it = json_object_iter(definition); it;
            ...
            for (size_t i = 0u; i < sizeof(allowed) / sizeof(allowed[0]); ++i)
                if (!strcmp(key, allowed[i]))
                    known = true;
            if (!known)
                return invalid(error, size, "unknown rule key");
        }
    }
    ...
    if (at_least && !json_is_object(at_least))
        return invalid(error, size, "at_least must be an object");
    if (confirm && !json_is_boolean(confirm))
        return invalid(error, size, "confirm must be a boolean");
    if (text && (!json_is_string(json_object_get(definition, "text")) ||
                 strlen(text) > SNAG_RULE_TEXT_MAX))
    ...
    if (rule->verb == SNAG_RULE_VERB_COUNT)
        return invalid(error, size, "unknown rule action");
    ...
    if (log) {
        if (strlen(log) > SNAG_RULE_TEXT_MAX)
            return invalid(error, size, "log format is too long");
    ...
    if (json_is_object(match) && json_object_size(match) > 0u) {
        ...
        for (void *it = json_object_iter((json_t *)match); it;
            ...
            if (!pred->pointer)
                return -1;
            if (regcomp(&pred->regex, json_string_value(value),
    ...
    if (json_is_object(at_least) && json_object_size(at_least) > 0u) {
        ...
        for (void *it = json_object_iter((json_t *)at_least); it;
            ...
            const char *key = json_object_iter_key(it);
            const json_t *value = json_object_iter_value(it);
            if (!pointer_valid(key) || !json_is_integer(value) ||
                json_integer_value(value) < 0)
                return invalid(error, size, "at_least needs /pointer integer pairs");
            rule->thresholds[rule->threshold_count].pointer =
                snag_strdup_checked(key, 256u);
            if (!rule->thresholds[rule->threshold_count].pointer)
                return -1;
...
snag_rules_compile(const json_t *definition, char *error, size_t size)
{
    ...
    if (!rules)
        return NULL;
    if (snag_json_digest(definition, rules->digest) < 0)
        goto fail;
    rules->chains = calloc(SNAG_CHAINS_MAX, sizeof(*rules->chains));
    rules->rules = calloc(SNAG_RULES_MAX, sizeof(*rules->rules));
    if (!rules->chains || !rules->rules)
        goto fail;
    if (chain_add(rules, "in") < 0 || chain_add(rules, "out") < 0 ||
        chain_add(rules, "event") < 0)
        goto fail;
    declared = json_object_get(definition, "chains");
    if (declared && !json_is_null(declared)) {
        if (!json_is_array(declared))
            goto fail;
        for (size_t i = 0u; i < json_array_size(declared); ++i) {
            const json_t *value = json_array_get(declared, i);
            if (!json_is_string(value) || chain_add(rules, json_string_value(value)) < 0)
                goto fail;
        }
    }
    rules->rule_count = list ? json_array_size(list) : 0u;
    for (size_t i = 0u; i < rules->rule_count; ++i)
        if (compile_rule(rules, json_array_get(list, i), i, error, size) < 0)
            goto fail;
    for (size_t i = 0u; i < rules->rule_count; ++i)
        if (rules->rules[i].verb == SNAG_RULE_JUMP &&
            chain_index(rules, rules->rules[i].target) == rules->chain_count)
            goto fail;
    return rules;
fail:
    if (!error[0])
        (void)snag_errorf(error, size, "invalid rule definition");
...
rule_matches(const struct snag_rule *rule, const json_t *envelope)
{
    for (size_t i = 0u; i < rule->threshold_count; ++i) {
        ...
        if (!json_is_integer(value) ||
            json_integer_value(value) < rule->thresholds[i].value)
            return 0;
    }
                ...
                struct snag_rule_verdict *verdict, char *error, size_t size)
{
    ...
    if (!snag_rules_boundary(boundary))
        return invalid(error, size, "invalid evaluation boundary");
    ...
    for (;;) {
        ...
        if (++verdict->visits > SNAG_RULE_VISITS_MAX)
            return invalid(error, size, "evaluation visit limit");
        int matched = rule_matches(rule, frame->envelope);
        if (matched < 0)
            return invalid(error, size, "rule matching failed");
        if (!matched)
            continue;
        ++verdict->matches;
        bool needs_host = rule->log || rule->confirm;
