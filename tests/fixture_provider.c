...
static int
set_response_id(struct snag_response_graph *graph, unsigned int cycle, const char *suffix)
{
    char id[128];
    if (snprintf(id, sizeof(id), "resp_fixture_%u_%s", cycle, suffix) < 0) return -1;
    return snag_response_graph_set_provider_id(graph, id);
}

static void
set_usage(struct snag_response_graph *graph, uint64_t input_tokens, uint64_t output_tokens)
{
                     ...
                     char *error, size_t error_size)
{
    ...
    if (strcmp(prompt, "empty_message_recovery") == 0 && cycle == 1u) {
        if (failure) failure->output_correction = SNAG_OUTPUT_CORRECTION_EMPTY;
        return snag_errorf(error, error_size, "%s", SNAG_EMPTY_OUTPUT_CORRECTION);
    }
    if (strcmp(prompt, "oversized_message_recovery") == 0 && cycle == 1u) {
        if (failure) failure->output_correction = SNAG_OUTPUT_CORRECTION_OVERSIZED;
    ...
    if (strcmp(prompt, SNAG_GOAL_CONTINUATION_TEXT) == 0) {
        ...
        if (strcmp(goal_prompt, "blocked goal") == 0 && cycle == 1u)
            return add_goal_call(graph, cycle, false, "block", "fixture dependency is unavailable");
    ...
    if (strcmp(prompt, "please create a persistent goal") == 0) {
        if (cycle == 1u) return add_goal_call(graph, cycle, true, NULL, "model-created goal");
    ...
    if (strstr(prompt, "network_tool")) {
        ...
        if (cycle == 2u) return add_irc_send_call(graph, cycle, 0u, "network tool complete");
    ...
    if (strcmp(prompt, "terminal_render") == 0) {
        static const char first[] = "alpha beta gamma delta-extraordinary ";
        static const char second_prefix[] = "zeta eta theta\nexplicit café ";
...
flood_done: snag_buf_free(&text);
    ...
    if (strcmp(prompt, "terminal_status") == 0) {
        static const char full[] = "status-first-fragment status-second-fragment";
        ...
        if (emit_fragment(&out, index, second, sizeof(second) - 1u) < 0) goto allocation;
        return 0;
    }
    ...
    if (strcmp(prompt, "typing_stream") == 0 || strstr(prompt, "network_view_stream")) {
        static const char full[] = "model-output-one model-output-two model-output-three";
    ...
    if (snag_string_in(prompt, "slow slow_utf8 queue_slow slow_resteer")) {
        if (cycle == 1u) {
            ...
            if (strcmp(prompt, "slow_utf8") == 0) {
                static const char euro[] = "€";
                if (emit_fragment(&out, 0u, euro + 1u, sizeof(euro) - 2u) < 0) goto allocation;
            }
        ...
        if (json_is_array(steering) && json_array_size(steering) != 0u) {
            json_t *last = json_array_get(steering, json_array_size(steering) - 1u);
            const char *text = snag_json_string(last, "text");
            char answer[1024];
            int written = text ? snprintf(answer, sizeof(answer), "steered: %s", text) : -1;
            if (written < 0 || (size_t)written >= sizeof(answer)) goto allocation;
    ...
    if (strcmp(prompt, "many_cycles") == 0) {
        if (cycle <= 129u) return add_call(graph, workspace, cycle, 0u, "fixture loop");
                 ...
                 json_t **result, char *error, size_t error_size)
{
    ...
    if (strstr(command, "managed steering wait") || strstr(command, "managed queue wait")) {
        ...
        *result = snag_tool_result_terminal(true, "fixture managed wait completed");
        return *result ? 0 : -1;
    }
...
{
    if (!strstr(text, "[IRC update id=")) return snag_buf_append(out, text, strlen(text));
