...
test_platform(void)
{
    ...
    int invalid_clock_error = errno;
    assert(clock_gettime((clockid_t)123456, &invalid_clock) == -1 && errno == invalid_clock_error);
#endif
...
#if defined(_WIN32) || defined(SNAJPAGENT_STATIC_UTF8) || defined(__ANDROID__)
...
run_base(int argc, char **argv)
{
    ...
    const char bounded_text[] = {'a', 'b'};
    assert(!snag_text_valid(bounded_text, 0u, 1u));
    assert(!snag_text_valid(bounded_text, 0u, 0u));
    ...
    assert(snag_buf_putc(&buf, 'a') == 0);
    assert(snag_buf_read(&buf, fileno(file)) == 0);
    assert(buf.len == 4u && !memcmp(buf.data, "ab\0c", 4u));
    assert(snag_buf_read(&buf, fileno(file)) == 0 && buf.len == 4u);
    assert(fseek(file, 0, SEEK_SET) == 0);
    assert(snag_buf_read(&buf, fileno(file)) == -2 && errno == EOVERFLOW);
    assert(buf.len == 4u && !memcmp(buf.data, "ab\0c", 4u));
