...
main(void)
{
    ...
    assert(snag_response_graph_from_json(&copy, encoded, error, sizeof(error)) == 0);
    assert(snag_response_graph_classify(&copy, &decision, error, sizeof(error)) == 0);
    assert(decision.outcome == SNAG_GRAPH_FINAL);
    assert(strcmp(snag_response_graph_item(&copy, 0).text, "pong") == 0);
    assert(snag_partial_public_validate(encoded, error, sizeof(error)) == 0);
    assert(json_array_append_new(encoded, json_deep_copy(json_array_get(encoded, 0u))) == 0);
    ...
    assert(snag_response_graph_set_provider_id(&graph, "resp_irc") == 0);
    {
        ...
        for (size_t i = 0u; i < sizeof(names) / sizeof(names[0]); ++i) {
            ...
            assert(snag_response_graph_add_call(&graph, item, call, names[i], arguments) == 0);
        }
    }
    assert(snag_response_graph_classify(&graph, &decision, error, sizeof(error)) == 0);
    ...
    assert(result);
    assert(snag_json_set_new(result, "max_output_tokens", json_integer(1)) == 0);
    assert(snag_tool_result_valid(result) == 0);
    assert(json_object_set_new(result, "max_output_tokens", json_integer(0)) == 0);
    assert(snag_tool_result_valid(result) < 0);
    assert(json_object_set_new(result, "max_output_tokens", json_integer(4000000001LL)) == 0);
    assert(snag_tool_result_valid(result) < 0);
    json_decref(result);
    ...
    /* A response may carry any number of calls and items. */
    {
