...
`/model` and its alias `/model list` read the persistent cache and never make a
provider request. If no cache exists yet, the first listing tries to seed it
from the local Codex `models_cache.json`: `$CODEX_HOME/models_cache.json` when
that absolute environment override is set, otherwise
`$HOME/.codex/models_cache.json`. The imported catalog is normalized under the
first configured provider because that is the provider used by an unqualified
selector. The source must be a bounded user-owned regular file with valid
Codex `models[].slug` data. Import is atomic and does not alter Codex's file.
If neither cache exists, listing reports how to populate one without trying
provider credentials.

The user alone decides that the cache is stale and refreshes all configured
providers with `/model cache`; there is no TTL or background refresh. There is
no assumed unauthenticated public catalog URL. The configured provider's
authenticated model endpoint is the authoritative online refresh source.
...
Parsing and saving a typed selector never contacts a provider or populates a
missing cache.
