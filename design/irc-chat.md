...
and never reused for a removed endpoint. A number names the endpoint, not one
...
Reuse the session, event-loop, tool, rendering, and managed-process abstractions
and omit duplication. Preserve required IRC, color, durability, reconnect,
steering, and UI behavior. `make sizecheck` reports source counts without limits
or per-file review thresholds.
