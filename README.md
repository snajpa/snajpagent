...
provider/model/reasoning catalog. A missing cache is seeded without network
access from Codex's `models_cache.json` when available (`$CODEX_HOME`, or
`$HOME/.codex` by default). `/model cache` explicitly refreshes every
configured provider through its authenticated model endpoint; there is no
automatic age check, so the user decides when it is stale. Every successful
catalog display ends with the cache update time.
...
cache. Selection itself performs no provider request. With only `MODEL`, the
first configured provider is used and the highest recognized advertised
effort is selected; if the cache has no effort metadata for it, the
current/configured effort is kept. A provider-native ID containing `/` can be
selected unambiguously by its displayed number.
