...
- Remove the eight-image request cap. Prepared image data remains bounded by the
  12 MiB request budget, so a session that accumulated more than eight images
  (for example several rendered document pages) is no longer rejected before
