...
let
  ...
  cflags = "-Os -g -D_NETBSD_SOURCE -fPIC ${if early then "-fno-stack-protector" else "-fstack-protector-strong"} ${lib.optionalString legacy "-femulated-tls"}";
  ...
  voiceRtc = import ./voice-rtc-cross.nix {
    ...
    sctpPatches = [ ./usrsctp-bsd-tailq-safe.patch ./usrsctp-netbsd-route-in6.patch ./usrsctp-legacy-arc4random.patch ];
  ...
  fontconfig = (autotoolsLibrary sourcePkgs.fontconfig [
    ...
    "--with-add-fonts=/usr/pkg/share/fonts"
